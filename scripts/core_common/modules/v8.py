...
def make():
  ...
  if ("windows" == base.host_platform()) and (config.option("vs-version") == "2019"):
    use_v8_89 = True
  if config.check_option("platform", "linux_arm64") or os.uname()[len(os.uname())-1]:
    use_v8_89 = True
  ...
  if config.check_option("platform", "linux_64") or os.uname()[len(os.uname())-1]: # this is a failsafe
