...
def make_xp():
  ...
  dev_path = os.path.join(config.option("vs-base-path"), "Common7/IDE")
  # old variant
  ...
  if config.check_option("platform", "win_64_xp"):
    ...
    if (-1 != config.option("config").lower().find("debug")) and not base.is_dir("win_64/debug"):
      base.run_as_bat(["call python v8/build/gyp_v8 -Dtarget_arch=x64", "call devenv v8/tools/gyp/v8.sln /Rebuild Debug"])
  ...
  if config.check_option("platform", "win_32_xp"):
    ...
    if (-1 != config.option("config").lower().find("debug")) and not base.is_dir("win_32/debug"):
      base.run_as_bat(["call python v8/build/gyp_v8", "call devenv v8/tools/gyp/v8.sln /Rebuild Debug"])
