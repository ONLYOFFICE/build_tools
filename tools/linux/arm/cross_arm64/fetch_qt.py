...
def make():
  ...
  if not base.is_file(os.path.dirname(__file__) + "/qt_binary_linux_arm64.7z"):
    base.download(qt_binary_url, os.path.dirname(__file__) + "/qt_binary_linux_arm64.7z")
    
  if not base.is_dir(qt_build_path):
    os.makedirs(qt_build_path)
    base.cmd("tar", ["-xf", os.path.dirname(__file__) + "/qt_binary_linux_arm64.7z", "-C", qt_build_path])
